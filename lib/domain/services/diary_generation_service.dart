...
class DiaryGenerationService {
  ...
  Future<DiaryGenerationResult> generateFromAlbum({
    required List<AppPhoto> photos,  // ← 改为传入完整照片对象
    required Pet pet,
  }) async {
    debugPrint('🔄 ========== 开始生成日记 ==========');
    ...
    debugPrint('✅ 使用日期: $photoDate (${selectedPhoto.photoTakenAt != null ? "拍摄日期" : "添加日期"})');

    // 生成日记内容
    final content = _generateContent(pet.name, photoDate);
    debugPrint('📝 日记内容长度: ${content.length} 字符');

    final result = DiaryGenerationResult(
      content: content,
      selectedPhotoId: selectedPhoto.id,
      selectedPhotoPath: selectedPhoto.localPath,  // ← 直接传递照片路径
      photoDate: photoDate,
  ...
  /// 生成日记内容
  String _generateContent(String petName, DateTime date) {
    final templates = [
      '''今天主人带我出去玩啦！
...
class DiaryGenerationResult {
  ...
  final String? selectedPhotoPath;  // ← 新增：直接返回照片路径
  ...
  factory DiaryGenerationResult.fromJson(Map<String, dynamic> json) {
    return DiaryGenerationResult(
      ...
      selectedPhotoPath: json['selected_photo_path'] as String?,  // ← 新增
